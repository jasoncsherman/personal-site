...
### 4. Public visitor / page-view counter
- **What it is:** A *visible* on-page counter (the retro "you are visitor #1,234" or per-page view count) shown to visitors — distinct from private analytics. Note: **Vercel Web Analytics is already set up (2026-06-15)** for Jason's own private numbers, so this idea is specifically about the *public-facing* display, not measurement.
- **Data source / state:** Needs somewhere to store and increment a count — the one thing V1's static, no-database, no-secrets model avoids (see Decisions + Deferred, which explicitly parks "analytics"). Two routes: **(a) third-party hit-counter service** (GoatCounter, counterapi, etc.) embedded as a badge — least maintenance, but an external dependency and their styling; **(b) self-hosted** — a Vercel serverless function plus a KV store (Vercel KV / Upstash Redis) — full control and on-brand styling, but reintroduces stateful backend + maintenance.
- **Feasibility:** Medium. Technically straightforward either way; the real cost is philosophical (breaks the no-backend purity) not technical.
- **Tone note:** A visible retro counter has a charming personal-web, anti-corporate vibe that arguably *fits* the "living archive / signal board" ethos better than a slick portfolio would — but can also read as a vanity metric. Deliberate call needed.
- **Open questions:** Site-wide total vs per-page counts? Third-party badge vs self-hosted KV? Where does it live — footer, Home, About? Is the charm worth breaking the static-no-backend principle for V1, or defer to V2?

### 5. Photo auto-import (display albums from an external source)
- **What it is:** Pull photos/albums from an external service and display them on the site, ideally without hand-creating each entry. Jason's preference is **Apple Photos**; open to easier alternatives.
- **Options (researched 2026-06-15):**
  - **Apple Photos** — can expose a Shared Album as a public iCloud web page (People → "Public Website"), but Apple blocks iframe-embedding it and offers no official API. Realistic outcome: a **link-out** to the iCloud album, not a natively-styled gallery on the site. No clean auto-pull.
  - **Flickr** — cleanest true auto-sync: real API + official embed codes + third-party widgets (e.g. SociableKit) that sync an album onto a site. Cost: photos live on Flickr; live-embed inherits their styling and depends on their uptime.
  - **Google Photos** — historically common, but Google restricted its Photos API in 2025 (apps can mostly only read media they uploaded), which broke many "embed my album" tools. Verify current state before relying on it.
- **Middle path (most on-brand):** a small script (run locally or scheduled) that **fetches new photos from a source and commits them into the repo** — downloads the image into the project's image folder and generates a Markdown stub with front matter pre-filled (date/EXIF), leaving `whyItMatters` blank for Jason to fill. Then the normal Astro build bakes them in. Keeps the static, no-secrets, version-controlled model; photos appear "automatically-ish" (pull-on-a-schedule, not live) but land as editable files, preserving the per-photo curation that defines the Photos section. Trade-offs: more upfront script work than pasting an embed, and not instant. Much smoother with an API-having source (Flickr) than with Apple (no clean API makes the fetch step fiddly).
- **Tension:** Live-embed services fight the site's curation ethos (each Photos entry carries a `whyItMatters` note); auto-dumping a raw album undercuts that. The middle path is automation that *feeds* curation rather than replacing it.
- **Open questions:** Accept Apple = link-out only, or switch source to Flickr for true integration? Live embed vs the commit-to-repo middle path? Worth the script-writing effort for V1, or defer?

### Resolved IA question: "field note" used to mean two things
- **The problem:** "Field note" was doing two unrelated jobs. It was (a) the name of a whole section — **Field Notes**, the renamed AI/learning log (schema: `tool`, `mode`, `confidence`, `keyLesson`; a dated record of the learning *process*) — and also (b) one of the `type` values in the **Writing** collection's enum (`Note | Essay | Reflection | Field note`). One Writing entry, `tools-that-earn-their-keep.md`, was tagged `type: "Field note"`, so it read as the same thing as the Field Notes section but lived under Writing.
- **Intended distinction:** Writing = outward-facing thinking (essays/notes/reflections about ideas and tools); Field Notes = inward-facing process (dated log of learning/building, especially the AI work). The collision blurred that line.
- **Resolution (2026-06-16):** Chose option (a) — reserved "Field Notes" for the learning log only. Dropped `"Field note"` from the Writing `type` enum (now `Note | Essay | Reflection` in `src/content.config.ts`) and retagged `tools-that-earn-their-keep.md` as `Reflection`. Each concept now has exactly one home. Also added a `status: draft | published` field to the `fieldNotes` schema (drafts hidden from production builds), and `HOW-TO-ADD-CONTENT.md` is now the single source of truth both content agents should follow. See [[LEARNINGS]] — "Working With Multiple Agents."
- **Status:** ✅ Resolved.
...
### 6. Field Notes publishing heatmap
- **What it is:** A GitHub-style calendar grid for the Field Notes archive, showing which dates have published notes. Unlike the GitHub contribution heatmap above, this is based on the site's own content dates and should feel more like a learning-log cadence than a code-activity metric.
...
### 8. More distinctive nav (revisit)
- **What it is:** The top nav currently uses "Direction A — The Index" (muted two-digit `01`–`08` numbers before each label; shipped 2026-06-16). It's a solid, low-risk default, but Jason isn't in love with any of the explored options and wants room to make the nav feel more distinctive later.
- **Already explored (2026-06-16, via style tile):** A — "The Index" (numbers, chosen); B — "Marginalia" (editorial glyphs `¶ § ◆ →`); C — "Toolbelt" (monoline SVG icons + labels). C is the literal-icons path but carries the most "generic AI website" risk and a recognizability problem for sections like Field Notes / Now / Bookshelf.
- **Directions to consider next:** combine A + B (number *and* glyph); a more custom/hand-drawn mark set; a typographic/interaction treatment (hover reveals, an animated active indicator) rather than icons; or a fundamentally different nav layout (vertical index/contents-page on desktop, not just mobile).
- **Re-entry condition:** revisit once there's real content across sections (so the nav is judged in context, not against placeholders), or sooner if a clear concept presents itself. Build any candidate as a scratch style tile first (the workflow that worked here).
- **Open questions:** Is "distinctive" worth added maintenance/complexity on a nav that already works? Should it stay non-pictographic to protect the editorial tone? Desktop vs. mobile treatments — same or different?
